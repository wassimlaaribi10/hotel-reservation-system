...
const Rooms = () => {
    ...
    const fetchEquipmentForRoom = async (roomId) => { 
        try { 
            const res = await API.get(`/rooms/${roomId}/equipment`); 
            setRoomEquipment(res.data); 
        } catch (err) { 
            console.error(err); 
        } 
    };
    ...
    const handleOpenEquipmentModal = async (room) => { 
    ...
    const addEquipmentToRoom = async (equipmentId) => { 
        if (!equipmentId) return; 
        try { 
            await API.post(`/rooms/${selectedRoom.id}/equipment/${equipmentId}`); 
            await fetchEquipmentForRoom(selectedRoom.id);
            await fetchRooms();
        } catch (err) { 
            alert('Erreur ajout'); 
        } 
    ...
    const removeEquipmentFromRoom = async (equipmentId) => { 
        try { 
            await API.delete(`/rooms/${selectedRoom.id}/equipment/${equipmentId}`); 
            await fetchEquipmentForRoom(selectedRoom.id);
            await fetchRooms();
        } catch (err) { 
    ...
    const resetForm = () => { 
        setFormData({ roomNumber: '', type: 'single', floor: '', capacity: '', description: '' }); 
        setEditingRoom(null); 
        setShowForm(false); 
    };
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try { 
            if (editingRoom) await API.put(`/rooms/${editingRoom.id}`, formData); 
    ...
    const handleActivate = async (id) => { 
        if (!window.confirm('Activer cette chambre ?')) return; 
        try { 
                    ...
                    <tbody>
                        {rooms.map(room => (
                            <tr key={room.id}>
                                ...
                                <td>{room.floor}</td>
                                <td>{room.capacity}</td>
                                {/* Description : centrée si vide, alignée à gauche sinon */}
                                <td className={!room.description ? 'description-empty' : ''}>
                                    {room.description || '—'}
                                </td>
                                <td className="equipment-cell">
                                    <div className="equipment-badges">
                                        {room.equipmentNames && room.equipmentNames.length > 0 ? (
                                            room.equipmentNames.map((eq, idx) => (
                                                <span key={idx} className="equipment-badge">{eq}</span>
                                            ))
                                        ) : (
                                ...
                                <td>{room.isActive ? 'Actif' : 'Inactif'}</td>
                                {isAdmin && (
                                    <td>
                                        ...
                                        {room.isActive ? 
                                            <button className="btn-action btn-deactivate" onClick={() => handleDeactivate(room.id)}>🔴</button> : 
                                            <button className="btn-action btn-activate" onClick={() => handleActivate(room.id)}>🟢</button>
                                        }
                                    </td>
                                )}
                            </tr>
                    ...
                    <div className="modal-container equipment-modal">
                        ...
                        <div className="modal-body">
                                ...
                                <ul className="equipment-list">
                                    {roomEquipment.length === 0 ? <li className="empty-tag">Aucun équipement</li> : 
                                        roomEquipment.map(eq => (
                                            <li key={eq.id}>
                                                {eq.name}
                                                <button className="remove-btn" onClick={() => removeEquipmentFromRoom(eq.id)}>Retirer</button>
                                            </li>
                                        ))
                                    }
                                </ul>
                            ...
                            <div className="equipment-section">
                                ...
                                <select onChange={(e) => addEquipmentToRoom(e.target.value)} defaultValue="" className="equipment-select">
